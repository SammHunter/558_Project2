...
This is a report about which model to use when trying to predict the number of social media shares of an article with a topic of, `r params[[2]]$topics`, based on a myriad of predictors.
...
* 1 Logistic Linear Model with all predictor varaibles.  
...
At the end of this report we decide on which model was the best for the `r params[[2]]$topics`.
...
Below we read in the data, add new variables, remove old ones, filter specifically for the `r params[[2]]$topics` topic, and then we go ahead and split the data into the test and train data sets for model training and testing. 
...
```{r}
filtered_data <- data %>% filter(topic == params[[2]]$topics) %>% select(!topic, -url)
...
The boxplot in the top left is a boxplot of the number of shares, and the boxplot on the right represents the distribution of log(shares). Even with the log transformations, we can still see some right skew-ness in number of shares. I have also included a histogram  of number of log(shares) faceted by the day the article was published and a scatter plot of number of log(shares) by how long ago they were published. What we must remember when we look at these graphs is how the natural log affects data. 



Here we take a look at the correlation of all variables for the `r params[[2]]$topics` topic with `shares`.
...
Then I build a contingency table between what day of the week an article was published and what quartile it ends up in. Finally I preform the Chi-Sqaured test.
```{r evan quartiles}
...
Within this dataset, we have 50 predictors. Three of these predictors are character variables - url of the article (url), the day the article was published (day), and the topic of the article (topic). We won't include url as a predictor since that seems to be more of an identifier variables rather than an attribute of the article. We also are analyzing the topics separately from each other so we won't include that in our linear model either. The rest of the predictors are numeric.


```{r}
# Build Full Model
lm_full_fit <- train(shares ~ . ,
              data = train_set,
...
par(mfrow = c(2, 2))
# residual plots from the full linear regression model
plot(lm_full_fit)
...
plot(lm_log_fit)
lm_log_pred <- predict(lm_log_pred, test_log)
...
The next linear model regression we created uses only the significant predictors from the full linear model. It includes nine predictors. I'm not too concerned about what the predictors stand for because we are not concerned about the interpretability of the model. We only care about prediction. We also use these same predictors in the random forest model. 

```{r}
lm_9_fit <- lm(shares ~ n_tokens_title + n_non_stop_words + num_hrefs + num_imgs +
...
For this next linear model, we started with the five of the highest correlation variables, and then added a few more that seemed to increase the models effectiveness. Including some interactions between them.
...
# Holding pattern, I'm thinking I can automate this process to choose the best random forest to then attempt cross validation on - this may go away though. 
# I ran these models first because the cross-validation takes forever
rfFit <- randomForest(shares~., data = train_set, mtry = ncol(train_set)/3, 
                      ntree = 200, importance = TRUE)
rfPred <- predict(rfFit, newdata = dplyr::select(train_set, -shares))
# MSE 2618.834
sqrt(mean((rfPred-train_set$shares)^2))


ranfor_sig <- randomForest(shares ~ n_tokens_title + n_non_stop_words + num_hrefs + num_imgs
             + average_token_length + kw_min_avg + kw_max_avg + kw_avg_avg + max_negative_polarity,
             data = train_set, mtry =  ncol(train_set)/3, ntree = 200, importance = TRUE)
ranfor_Pred <- predict(ranfor_sig, newdata = dplyr::select(train_set, -shares))
# 2686.744
sqrt(mean((ranfor_Pred-train_set$shares)^2))


ranfor_sig_sub <- randomForest(shares ~ n_tokens_title + n_non_stop_words + num_hrefs + num_imgs
             + average_token_length + kw_avg_avg + max_negative_polarity,
             data = train_set, mtry =  ncol(train_set)/3, ntree = 200, importance = TRUE)
ranfor_Pred_sub <- predict(ranfor_sig_sub, newdata = dplyr::select(train_set, -shares))
# 2618.21, pretty small comparatively so this is what I'm going to use for the cross-validation training
sqrt(mean((ranfor_Pred_sub-train_set$shares)^2))
...
rf_SigPred_Pred <- predict(rf_SigPred, newdata = dplyr::select(train_set, -shares))
...
#rf_AllPred
#rf_All_Pred <- predict(rf_AllPred, newdata = dplyr::select(train_set, -shares))
#rf_MSE <- sqrt(mean((rf_All_Pred-train_set$shares)^2))
# RMSE 3629.05
#rf_MSE
#plot(rf_AllPred)
...
# sometimes you need to run this to get out of the cluster/run without the cluster
...
Below I create two boosted tree models, the first is a model with every predictor variable included, the second is with some selected variables to try and improve on the model with all predictors. For both models I used 10 fold cross validation, number of trees ranging from 100 to 500, an interation depth from 1 to 10, shrinkage of 0.1, and min obs in node of 10.
...
# Predict full model
boost_tree_pred <- predict(boost_tree_full_fit,test_set)
...
To determine the best model to use for predicting the amount of shares of an article about (will insert params$topic here) we have decided to measure them against each other using the Root Mean Square Error (RMSE). To read about RMSE I suggest [this link](https://www.statisticshowto.com/probability-and-statistics/regression-analysis/rmse-root-mean-square-error/) as it provides a general explanation of what it is and also has a video!


Before we can determine the best model we have to compare them. So that is what we do here, first we establish the name of each model, and then we add the name and their corresponding RMSEs to a tibble for comparison!
```{r model selection}

# Names of models
model_names <- c("Full Linear Model", "Selected Linear Model","Full Boosted Tree Model", "Selected Boosted Tree Model", "Random Forest Model") # these can be added to, just getting the first try at this down

# RMSE values
model_rmses <- c(lm_full_rmse,lm2_rmse,boost_tree_full_rmse,boost_tree_rmse,rf_MSE)  # these can be added to
...
From this table we can see that for the `r params[[2]]$topics` topic the best model is the `r as.character(model_comp[1,1])` with an RMSE value of `r round(model_comp[1,2],digits = 4)`
...
output_file <- paste0(subj_topics, ".html")

## First have to build this function
params <- lapply(subj_topics, FUN = function(x){list(team = x)})
