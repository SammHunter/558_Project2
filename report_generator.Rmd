...
train_transformed <- predict(preProcValues, train_set)
test_transformed <- predict(preProcValues, test_set)

# Build Full Model
lm_full_fit <- lm(shares ~ . , data = train_transformed)

lm_full_pred <- predict(lm_full_fit,newdata = test_transformed)
# check RMSE of prediction
lm_full_rmse <- sqrt(mean((lm_full_pred-test_transformed$shares)^2))
...
lm_9_pred <- predict(lm_9_fit,newdata = test_transformed)
# check RMSE of prediction
lm_9_rmse <- sqrt(mean((lm_full_pred-test_transformed$shares)^2))
```
...
```{r evan lm}
# Fit model
lm2_fit <- lm(shares ~ LDA_02 + LDA_03 + num_imgs*n_tokens_content  + average_token_length+
...
lm2_pred <- predict(lm2_fit,newdata = test_transformed)

# check RMSE of prediction
lm2_rmse <- sqrt(mean((lm2_pred-test_transformed$shares)^2))
