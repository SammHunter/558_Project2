...
Below we read in the data, add new variables, remove old ones, filter specifically for (topic), and then we go ahead and split the data into the test and train datasets for model training and testing. 
...
# Filter data for specific topic, and then remove topic and url from dataset.
filtered_data <- data %>%
  filter(topic == "world") %>%  # this is where filtering for params will be needed
  select(-url,-topic)

# setting the seed
  set.seed(214)

  # splitting it into 70% for training and 30% for testing
  train <- sample(1:nrow(filtered_data), size = nrow(filtered_data)*0.7)
  test <- setdiff(1:nrow(filtered_data), train)

  # Actually subsetting
  train_set <- filtered_data[train,]
...
topic_selection <- function(subj, ...){

  # condition logic here where we either say if subj = world then do, else, ect
  subject <- data %>% filter(topic == "world") %>% select(!topic, -url)
  
  # split the data into a training and test set 
  set.seed(214)

  # splitting it into 80% for training and 20% for testing
  train <- sample(1:nrow(subject), size = nrow(subject)*0.7)
  test <- setdiff(1:nrow(subject), train)

  # Actually subsetting
  train_set <- subject[train, 1:48]
  test_set <- subject[test, ]

  # creating a log data subset as well
  train_log <-train_set%>% mutate(shares = log(shares))
      getSummaries(train_set$shares)
  test_log <-test_set%>% mutate(shares = log(shares))
  
  return(list(subject, train_set, test_set, train_log, test_log))
}

topic_selection(world)
...
num_cols <- select_if(world, is.numeric)
...
# 0 to 843300 range, mean is 3395.38, median 1400
shares
...
ggplot(data = train_log, aes(shares, timedelta)) +
  geom_point(aes(color = day))

```


We included some graphs about data where we have changed the response by applying the natural log function to the response. By transforming the response this way, we can actually see what sort of patterns appear in the response variables. As you can see, without the log transformation, we can't even see anything but outliers in the box plot, but once we apply the log transformation, we actually get a box plot, admittedly also with a lot of outliers. 
...
# Horrible
lm_full <- lm(shares~., data=train_set)
summary(lm_full)
# also horrible
par(mfrow = c(2, 2))
plot(lm_full)
...
# best model I found
lm_log_full <- lm(shares~., data = train_log)
summary(lm_log_full)
# still not a great linear model plot, but a heck of a lot better than before
plot(lm_log_full)
lm_log_pred <- predict(lm_log_full, test_log)
lm_log_MSE <- sqrt(mean((lm_log_pred-test_log$shares)^2))
# Seems high 0.8134
lm_log_MSE


### Not as good model as above (based on Adj-R Squares)
#lm_sig_subset <- lm(shares ~ n_tokens_title + n_non_stop_words + num_hrefs + num_imgs
#             + average_token_length + max_negative_polarity, 
#             data = train_set)
#summary(lm_sig_subset)
...
Might continue to tinker with this, but this is as good as its getting rn.
...
From this table we can see that for the (insert params$topic here) topic the best model is the `r as.character(model_comp[1,1])` with an RMSE value of `r round(model_comp[1,2],digits = 4)`
