...
world <- data %>% filter(topic == "world") %>% select(!topic, -url)
...
train <- sample(1:nrow(world), size = nrow(world)*0.7)
test <- setdiff(1:nrow(world), train)

# Actually subsetting
train_set <- world[train, 1:48]
test_set <- world[test, ]
...
lm_log_MSE <- sqrt(mean((lm_log_MSE-test_log$shares)^2))
lm_log_MSE
...
When we look at the residual plots from the linear regression we do have a goal in mind. For the residual plot (top left), we want to see **no** trends. 
...
rf_SigPred <- train(shares ~ n_tokens_title + n_non_stop_words + num_hrefs 
             ...
             data = train_set, method = "rf", 
              ...
              preProcess = c("center", "scale"), 
              tuneGrid = data.frame(mtry = c(1, 7, 8, 16, 17, 18)))
