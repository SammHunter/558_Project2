...
getSummaries(world$shares)

ggplot(data = world, aes(shares, timedelta)) +
...
lm_6_sig <- lm(shares ~ n_tokens_title + n_non_stop_words + num_hrefs + num_imgs
...
rf_LDA <- train(shares ~ LDA_00+LDA_01+LDA_02+LDA_03+LDA_04, data = train_set, method = "rf", 
              trControl = trainControl
               (method = "repeatedcv", number = 5, repeats = 3), 
              preProcess = c("center", "scale"), 
              tuneGrid = data.frame(mtry = 1:15))
# Smallest RMSE is 5698.148 - so, so bad
rf_LDA

rf_SigPred <- train(shares ~ n_tokens_title + n_non_stop_words + num_hrefs + num_imgs
             + average_token_length + kw_min_avg + kw_max_avg + kw_avg_avg + max_negative_polarity,
